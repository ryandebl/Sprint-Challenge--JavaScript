...
const contactInfo = [];
graduates.forEach((arrItem)=>{
  contactInfo.push(`${arrItem.first_name} ${arrItem.email}`);
})
...
const unisWithUni = [];
graduates.forEach((arrItem) => unisWithUni.push(
  arrItem.university.includes('Uni') ? arrItem : null
))
...
/* Request 1: .forEach() test for push
...
The zoos need a list of all their animal's names (animal_name only) converted to lower case. Using map, create a new array of strings named lowCaseAnimalNames, each string following this 
